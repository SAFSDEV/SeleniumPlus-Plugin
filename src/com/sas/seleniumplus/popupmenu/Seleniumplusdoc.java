...
public class Seleniumplusdoc extends AbstractHandler{
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			...
			String relativeURI = CommonLib.getPreferenceStore().getString(PreferenceConstants.SAFS_DOC_SELENIUMPLUS).trim();
			URL url = new URL(StringUtils.concatURL(baseURL, relativeURI));

			if(!NetUtilities.isHttpURLReachable(url, 1000)){
				//If the SE+ document on web is not accessible, try to open the local file.
				File localFile = new File(Activator.seleniumhome + "/doc/SeleniumPlus-Welcome.html");
				if(localFile.exists()){
					url = localFile.toURI().toURL();
				}
			}

			PlatformUI.getWorkbench().getBrowserSupport().createBrowser("SeleniumPlusDoc").openURL(url);
		} catch (Exception e) {
