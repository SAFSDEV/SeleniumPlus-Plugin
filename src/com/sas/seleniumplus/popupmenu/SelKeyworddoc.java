...
String selenv = System.getenv(BaseProject.SELENIUM_PLUS_ENV);
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			...
			String relativeURI = CommonLib.getPreferenceStore().getString(PreferenceConstants.SAFS_DOC_KEYWORDS).trim();
			URL url = new URL(StringUtils.concatURL(baseURL, relativeURI));

			if(!NetUtilities.isHttpURLReachable(url, 1000)){
				//If the SE+ keywords document on web is not accessible, try to open the local file.
				File localFile = new File(Activator.seleniumhome + "/doc/SeleniumPlusKeywords.htm");
				if(localFile.exists()){
					url = localFile.toURI().toURL();
				}
			}

			PlatformUI.getWorkbench().getBrowserSupport().createBrowser("SeleniumPlusKeywordDoc").openURL(url);
		} catch (Exception e) {
