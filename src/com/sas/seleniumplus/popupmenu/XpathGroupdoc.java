...
public class XpathGroupdoc  extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			...
			URL url = null;
			File localFile = new File(Activator.seleniumhome + "/doc/Xpath_Css_selenium_Locators_groups_1_0_2.pdf");
			if(localFile.exists()){
				url = localFile.toURI().toURL();
			}else{
				String baseURL = CommonLib.getPreferenceStore().getString(PreferenceConstants.SAFS_DOC_BASE_URL);
				String relativeURI = CommonLib.getPreferenceStore().getString(PreferenceConstants.SAFS_DOC_XPATH_GROUP);
				url = new URL(StringUtils.concatURL(baseURL, relativeURI));
			}

			PlatformUI.getWorkbench().getBrowserSupport().createBrowser("XPATHDOCByGroups").openURL(url);
